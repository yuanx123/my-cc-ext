...
- **注释规范** — 类/方法 JavaDoc（中文）、关键逻辑有注释；**新增/改动方法的注释遵守下方「文档注释完整性」与「注释精简口径」**；无废话注释；`TODO` / `FIXME` 标注明确
- **方法注释结构规范** — 方法 JavaDoc 先一句话职责，再以**编号逻辑步骤（1. 2. 3. …）**描述主流程、分支归属与边界；禁止把判定规则、边界、TODO/设计代号等揉成一大段 `<p>` 叙述；方法体关键节点用简短内联注释，不写长句小作文
- **注释精简化（必查项，违者报 WARNING 要求清理）** — 注释只写代码**不自明**的意图/边界/取舍；**违禁形态（逐一扫描，命中即报告）**：① 类级「平行于某场景 / 差异在… / 业务表 xxx（键列=…）、附件 obj_type 等」流水账式说明；② 在代码注释里写**设计代号 / 文档内部引用**（如 `D1~D7`、`TODO-T1~Tn`、`见 design-xxx §x`）；③ 逐行复述代码本身的废话注释（`// 把 X 赋给 Y` 之类）；`TODO`/`FIXME` 仅允许真实待办且写明内容
- **简洁性规范** — **新增/改造方法与 SQL 遵守下方「代码简洁性」口径**：入参最少且内聚、优先复用既有查询/对象、SQL 谓词不冗余、较独立的分支/计算抽私有方法
- **方法内聚规范** — **新增/改造方法遵守下方「按逻辑节点抽取私有方法（MUST）」**：方法内出现多个可命名的独立子任务/逻辑节点即应抽取，**主动检查、不得等用户/审查人指出**
...
#### 文档注释完整性（新增/改动方法与字段，MUST）

审查**新增/改动方法**的文档注释（Javadoc）时，对照以下完整性口径逐条核查（本细目查注释**是否齐全**，与下方「注释精简口径」（查**是否精简**）配套，同为本次新增代码的强制审查项）：

- **@param 与签名一一对应** — 签名每个参数都有 `@param` 且带非空中文说明，不遗漏、不残留已删除/改名参数；`@param` 存在但说明为空视为缺失
- **@return / @throws** — 非 `void` 方法必须有带说明的 `@return`（`void` 方法不写）；签名声明异常或显式抛业务异常处补 `@throws`
- **措辞说明“做什么/为什么”** — 叙述方法职责与设计意图，**可按逻辑节点列步骤（1）… 2）… 3）…）**，属良好可读写法、不算冗长；「精简」指不逐行复述琐碎实现（判断/三目/循环过程、重复赋值等内部细节）、不与方法名或正文做无信息重复；中文、简洁明确
- **@Override 特例** — 实现类 `@Override` 方法在接口已有完整注释时可只写 `{@inheritDoc}`，不算缺失；接口无完整注释或实现类独有 public 方法须写完整 JavaDoc
- **字段注释** — 新增字段须有一句话中文注释说明业务语义并带量纲/单位（如「违退数（违约退租合同数，个）」），不允许无注释或仅复述类型

> code-review 执行时须按上述口径核查本次新增/改动方法的文档注释，命中缺 `@param` / `@return` / `@throws`、`@param` 与签名不一致、`@param` / `@return` 说明为空、逐行复述琐碎实现（重复赋值等内部细节）或与方法名做无信息重复时，**必须附 `文件:行`** 列报（注释缺失/标签不全/空说明归 WARNING；仅琐碎内部细节复述、无信息重复归 INFO；按逻辑节点列步骤不算冗长，不列报）。
...
- **避免无效语境**：注释与 Javadoc 只描述该方法/字段自身的语义、用途、参数与边界；**禁止写入对读者不可见的需求内部代号或上下文**（如「T2/T3 用」「本次违退改造用」「某专项对接用」等），以及任何读者仅凭代码无法理解的项目内部称谓；注释措辞应能脱离本需求背景独立成立

> code-review 在执行时须检查本次新增代码是否符合上述口径，发现冗余/过程性注释列为风格问题反馈（归入 WARNING / INFO 级）；命中「避免无效语境」（注释/Javadoc 写入需求内部代号、上下文或仅凭代码不可解的项目内部称谓）**必须附 `文件:行`** 列报。
...
#### 按逻辑节点抽取私有方法（代码结构/方法内聚，MUST）

审查**新增/改造方法**的方法内聚性时，对照以下口径逐条核查（本条为「代码简洁性 → 逻辑收敛」的**显性强化**，同为本次新增代码的强制审查项）：

- **可命名的独立子任务/逻辑节点即应拆** — 单方法内部若出现多个**可命名的独立子任务/逻辑节点**（例如「先查 A → 再查 B → 再替换/新增」，各节点有独立输入输出、可复用、可单测），应拆为独立私有方法后组合调用，不得全部线性堆叠在单个方法内
- **触发「应抽取」的信号** — 方法体明显过长或缩进过深、一行内夹杂多个无关注释段（本可按节点分述的），即触发「应抽取」
- **主动检查、不等人指出** — **code-review 主动检查本条，不得等用户/审查人指出后才报告**；命中项**必须附 `文件:行`**，并建议给出拆分出的私有方法名

> code-review 执行时须主动核查本次新增/改造方法的方法内聚，命中「多个可命名独立子任务/逻辑节点未拆」「方法过长/缩进过深/注释段混杂仍不抽」即列报（附 `文件:行` + 建议拆分出的方法名；通常归 WARNING，可读性/可维护性显著受损或影响单测时建议优先重构）。
