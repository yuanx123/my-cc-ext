...
- **按逻辑节点抽取私有方法（MUST，编码即自查）**：方法内出现多个**可命名的独立子任务/逻辑节点**（如「先查 A → 再查 B → 再替换/新增」）编码时就拆为独立私有方法组合调用；方法体明显过长或缩进过深、一行内夹杂多个无关注释段的，即触发「应抽取」。**不得等用户/审查人指出后才抽**——编码阶段自查，命中的拆分同步回写 plan 的任务结构；后续 `code-review`（第六步）按「维度 A 按逻辑节点抽取私有方法」主动检查（见「约束」同名 MUST）
...
- **文档注释完整性（Javadoc，MUST）**：新增/改动方法须有完整文档注释——签名每个参数都有带说明的 `@param`、非 `void` 方法有带说明的 `@return`、声明异常处补 `@throws`；实现类 `@Override` 方法在接口已有完整注释时可只写 `{@inheritDoc}`，不算缺失；注释措辞说明“做什么/为什么”，可按逻辑节点列步骤（1）… 2）… 3）…）（良好写法、不算冗长）；「精简」指不逐行复述琐碎实现（判断/三目/循环过程、重复赋值等内部细节）、不与方法名做无信息重复；字段注释一句话含量纲。编码自检与委托 `code-review` Agent（第六步）均按此核查，缺失/不一致须附 `文件:行` 列报
- **避免无效语境（MUST）**：注释与 Javadoc 只描述该方法/字段自身的语义、用途、参数与边界，**禁止写入对读者不可见的需求内部代号或上下文**（例如「T2/T3 用」「本次违退改造用」「某专项对接用」等），以及任何读者仅凭代码无法理解的项目内部称谓；注释措辞应能脱离本需求背景独立成立。编码自检与委托 `code-review` Agent（第六步）均按此核查，命中附 `文件:行` 列报
...
- **按逻辑节点抽取私有方法（MUST，编码即自查）**：单方法内部若出现多个**可命名的独立子任务/逻辑节点**（例如「先查 A → 再查 B → 再替换/新增」，各有独立输入输出、可复用、可单测），编码时就应拆为独立私有方法后组合调用；方法体明显过长或缩进过深、一行内夹杂多个无关注释段的，即触发「应抽取」。**不得等用户/审查人指出后才抽**——编码阶段自查，委托 `code-review`（第六步）时其按「维度 A 按逻辑节点抽取私有方法」主动检查；命中附 `文件:行`，并给出建议拆分出的方法名
