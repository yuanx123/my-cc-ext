...
- [x] 增加 Git 市场和发布门禁的失败测试，确认缺少入口及工作流时失败。
- [x] 源码市场使用 `git-subdir` 指向 `codex-dist` 的完整插件，保持共享技能目录隔离。
- [x] 增加正式 Release/默认分支手动触发的构建与发布任务，沿用打包脚本。
- [x] 更新安装说明，区分首次发布前提、Git 安装和本地开发安装。
- [x] Codex 与共享内容测试、打包、版本、可移植性及 JavaDoc 检查通过。
- [x] YAML 解析和 Bash 语法检查通过；临时 Git 仓库验证首次发布、更新、无变化重跑、隐藏文件和源码保留。
- [ ] 合并至默认分支后触发 GitHub 首次发布；未自动合并主分支，也未宣称线上安装已经可用。
